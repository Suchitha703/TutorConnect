...
$stmt = $db->conn->prepare("SELECT * FROM users WHERE user_id = ? AND user_type = 'student'");
...
// Get or create request between tutor and student
$stmt = $db->conn->prepare("SELECT * FROM requests WHERE tutor_id = ? AND student_id = ? ORDER BY created_at DESC LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($message)) {
        ...
        $stmt = $db->conn->prepare("INSERT INTO messages (sender_id, receiver_id, request_id, message_text) 
                                   VALUES (?, ?, ?, ?)");
        $stmt->execute([$tutorId, $studentId, $requestId, $message]);
    }
}
...
$stmt->execute([$tutorId, $studentId, $studentId, $tutorId]);
...
<body class="bg-gray-100">
    <div class="flex flex-col h-screen">
        ...
        <div class="bg-white shadow p-4 flex items-center">
            ...
            <img src="../assets/images/student.jpg" alt="Student" class="w-10 h-10 rounded-full mr-3">
            <div>
                <h2 class="font-bold"><?= htmlspecialchars($student['full_name']) ?></h2>
                <p class="text-xs text-gray-600">
                    <?= $request ? ucfirst($request['status']) : 'No active request' ?>
                </p>
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4" id="chat-messages">
            ...
            <?php else: ?>
                <?php foreach ($messages as $message): ?>
                    <div class="flex <?= $message['sender_id'] == $tutorId ? 'justify-end' : 'justify-start' ?>">
                        <div class="<?= $message['sender_id'] == $tutorId ? 'bg-blue-500 text-white' : 'bg-white' ?> rounded-lg p-3 max-w-xs md:max-w-md shadow">
                            <p><?= htmlspecialchars($message['message_text']) ?></p>
                            <p class="text-xs mt-1 <?= $message['sender_id'] == $tutorId ? 'text-blue-100' : 'text-gray-500' ?>">
